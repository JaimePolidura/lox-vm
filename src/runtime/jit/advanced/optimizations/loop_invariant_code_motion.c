...
static void initialization_block(struct ssa_block *, void *);
static void move_loop_invariants(struct licm *);
static bool can_be_checked_for_loop_invariant_control_node(struct ssa_control_node *node);
...
static void move_up_loop_invariat(struct licm *, struct ssa_control_node *);
...
static void move_loop_invariants(struct licm * licm) {
    while (!is_empty_stack_list(&licm->pending)) {
        ...
        if (can_be_checked_for_loop_invariant_control_node(pending_control_node)) {
...
static bool try_move_up_loop_invariant_data_node(
    ...
    if(is_data_node_loop_invariant(consumer_struct->licm, consumer_struct->control_node, current_data_node)) {
        move_up_loop_invariat(consumer_struct->licm, consumer_struct->control_node);
...
static void initialization_block(struct ssa_block * current_block, void * extra) {
