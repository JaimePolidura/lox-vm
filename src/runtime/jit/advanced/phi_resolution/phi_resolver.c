...
static bool is_define_phi_node(struct ssa_control_node *);
...
static void resolve_phi_control(struct pr * pr, struct ssa_control_node * control_node) {
    ...
    if (control_node->type == SSA_CONTROL_NODE_TYPE_DEFINE_SSA_NAME && !is_phi_node) {
...
static bool is_define_phi_node(struct ssa_control_node * control_node) {
    return control_node->type == SSA_CONTROL_NODE_TYPE_DEFINE_SSA_NAME &&
        ((struct ssa_control_define_ssa_name_node *) control_node)->value->type == SSA_DATA_NODE_TYPE_PHI;
}
