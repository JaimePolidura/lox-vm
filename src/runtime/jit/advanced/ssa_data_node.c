...
void get_used_locals_consumer(struct ssa_data_node * _, struct ssa_data_node ** __, struct ssa_data_node * current_node, void * extra) {
...
void for_each_ssa_data_node(struct ssa_data_node * node, void * extra, ssa_data_node_consumer_t consumer) {
    for_each_ssa_data_node_recursive(NULL, NULL, node, extra, consumer);
}
    ...
    switch(current_node->type){
        case SSA_DATA_NODE_TYPE_INITIALIZE_STRUCT: {
            ...
            for(int i = 0; i < init_struct->definition->n_fields; i++){
                for_each_ssa_data_node_recursive(current_node, &init_struct->fields_nodes[i], init_struct->fields_nodes[i], extra, consumer);
            }
            break;
        }
        case SSA_DATA_NODE_TYPE_GET_STRUCT_FIELD: {
            ...
            for_each_ssa_data_node_recursive(current_node, &get_struct_field->instance_node, get_struct_field->instance_node, extra, consumer);
            break;
        }
        case SSA_DATA_NODE_TYPE_BINARY: {
            struct ssa_data_binary_node * binary_node = (struct ssa_data_binary_node *) current_node;
            for_each_ssa_data_node_recursive(current_node, &binary_node->left, binary_node->left, extra, consumer);
            for_each_ssa_data_node_recursive(current_node, &binary_node->right, binary_node->right, extra, consumer);
            break;
        }
        case SSA_DATA_NODE_TYPE_UNARY: {
            struct ssa_data_unary_node * unary = (struct ssa_data_unary_node *) current_node;
            for_each_ssa_data_node_recursive(current_node, &unary->unary_value_node, unary->unary_value_node, extra, consumer);
            break;
        }
        case SSA_DATA_NODE_TYPE_CALL: {
            ...
            for_each_ssa_data_node_recursive(current_node, &call_node->function, call_node->function, extra, consumer);
            for(int i = 0; i < call_node->n_arguments; i++){
                for_each_ssa_data_node_recursive(current_node, &call_node->arguments[i], call_node->arguments[i], extra, consumer);
            }
            break;
        }
        case SSA_DATA_NODE_TYPE_GET_ARRAY_ELEMENT: {
            struct ssa_data_get_array_element_node * get_array_element = (struct ssa_data_get_array_element_node *) current_node;
            for_each_ssa_data_node_recursive(current_node, &get_array_element->instance, get_array_element->instance, extra, consumer);
            break;
        }
        case SSA_DATA_NODE_TYPE_INITIALIZE_ARRAY: {
            ...
            for(int i = 0; i < init_array->n_elements && !init_array->empty_initialization; i++){
                for_each_ssa_data_node_recursive(current_node, &init_array->elememnts_node[i], init_array->elememnts_node[i], extra, consumer);
            }
            break;
        }
        default:
