...
void set_unmarked_bitmap(struct mark_bitmap * mark_bitmap, uint64_t * address) {
    uint8_t index_in_slot = ((uint64_t) address) >> 61;
    mark_bitmap->start[address - mark_bitmap->start_address] ^= index_in_slot << 0x01;
}

void set_marked_bitmap(struct mark_bitmap * mark_bitmap, uint64_t * address) {
    uint8_t index_in_slot = ((uint64_t) address) >> 61;
    mark_bitmap->start[address - mark_bitmap->start_address] |= index_in_slot << 0x01;
}

bool is_marked_bitmap(struct mark_bitmap * mark_bitmap, uint64_t * address) {
