...
static inline void increase_n_function_calls(struct function_object * function) {
    ...
    //a thread increments the function calls & other thread initializes the function profile operand (since these datastructures
    //are placed in a union, it will corrupt the profile operand)
    //By doing "==" comparation, only one thread will observe the function calls tao be the same value_as MIN_CALLS_TO_PROFILE
    //In this way only one function profile operand will get allocated
