...
static void set_array_element(struct call_frame * call_frame) {
    ...
    if (IS_OBJECT(new_value) && get_barriers_gc_alg().write_barrier != NULL) {
        get_barriers_gc_alg().write_barrier(array, AS_OBJECT(new_value));
    }
}
...
static void set_struct_field(struct call_frame * call_frame) {
    ...
    if (IS_OBJECT(new_value) && get_barriers_gc_alg().write_barrier != NULL) {
        get_barriers_gc_alg().write_barrier(instance, AS_OBJECT(new_value));
    }
}
